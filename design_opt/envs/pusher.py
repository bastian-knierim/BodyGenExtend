...
class PusherEnv(MujocoEnv, utils.EzPickle):
    def __init__(self, cfg, agent):
        ...
        self.box_joint_id = self.model.joint_name2id("box_joint") # additional body positions
        self.box_qpos_adr = self.model.jnt_qposadr[self.box_joint_id]
        self.box_id = self.model.body_name2id("box")
        self.arrow_id = self.model.body_name2id("arrow")
        self.robo_id = self.model.body_name2id("0")
    ...
    def step(self, a):
        ...
        if self.stage == 'skeleton_transform':
            ...
            if self.cur_t == self.cfg.skel_transform_nsteps:
                self.subtree_body_ids = self.get_subtree_body_ids() #  document subtree for contact
        ...
        # execution stage
            ...
            box_pos_bef = self.data.qpos[self.box_qpos_adr: self.box_qpos_adr + 3]
            if not self.has_contact_with_box:
                rob_pos_bef = self.sim.model.body_pos[self.robo_id]
                rob_box_dis_bef = np.linalg.norm(box_pos_bef - rob_pos_bef)
            box_goal_dis_bef = np.linalg.norm(box_pos_bef - self.goal_pos)

            try:
                if not self.has_contact_with_box:
                    self.check_contact_with_box()
            ...
            box_pos_aft = self.data.qpos[self.box_qpos_adr: self.box_qpos_adr + 3].copy()
            if not self.has_contact_with_box:
                rob_pos_aft = self.sim.model.body_pos[self.robo_id]
                rob_box_dis_aft = np.linalg.norm(box_pos_aft - rob_pos_aft)
                scale = self.cfg.reward_specs.get('exec_reward_scale', 1.0)
                reward = (rob_box_dis_aft - rob_box_dis_bef) / self.dt
            else:
                box_goal_dis_aft = np.linalg.norm(box_pos_aft - self.goal_pos)
                scale = self.cfg.reward_specs.get('exec_reward_scale', 1.5)
                reward = (box_goal_dis_aft - box_goal_dis_bef) / self.dt
            ...
            goal_radius = done_condition.get('goal_radius', 0.05)
            ...
            if (np.linalg.norm(self.goal_pos - box_pos_aft) < goal_radius):
                termination = True
                reward += self.cfg.reward_specs.get('goal_reward', 100.0)
            ob = self._get_obs()
    ...
    def get_sim_obs(self):
        ...
        if self.control_nsteps == 1:
            assert np.count_nonzero(obs[:, :5]) == self.model.nq + self.model.nv - 1 - self.cfg.obs_specs.get("add_bodys", "0") * 13
        return obs
    ...
    def get_subtree_body_ids(self):
        return {self.model.body_name2id(body.name) for body in self.robot.bodies}

    def check_contact_with_box(self):
        for i in range(self.data.ncon):
            contact = self.data.contact[i]
            b1 = self.model.geom_bodyid[contact.geom1]
            b2 = self.model.geom_bodyid[contact.geom2]
            if ((b1 == self.box_id and b2 in self.subtree_body_ids) or
                    (b2 == self.box_id and b1 in self.subtree_body_ids)):
                self.has_contact_with_box = True
                break
    ...
    def reset_model(self):
        ...
        self.has_contact_with_box = False
        self.subtree_body_ids = None

        self.data.qpos[self.box_qpos_adr: self.box_qpos_adr + 3] = [np.random.uniform(-20,20), np.random.uniform(-20,20), 2.5]
        self.goal_pos = [np.random.uniform(-20,20), np.random.uniform(-20,20), 4.5]
        self.sim.model.body_pos[self.arrow_id] = self.goal_pos
