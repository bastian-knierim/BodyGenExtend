...
class AntFlipEnv(MujocoEnv, utils.EzPickle):
    ...
    def step(self, a):
        ...
        # execution stage
            ...
            self._cache_box_addrs()
            box_rot_bef = self.data.qpos[self.box_qpos_adr + 3 : self.box_qpos_adr + 7]
            box_rot_dist_bef = ang_dist(array2quat(self.goal_rot), array2quat(box_rot_bef))

            if self.task_specs.get('mov_goal', False):
                box_goal_dist_bef = np.linalg.norm(box_pos_bef - self.goal_pos)
            elif self.task_specs.get('box_cont_cost', False):
            ...
            box_rot_aft = self.data.qpos[self.box_qpos_adr + 3 : self.box_qpos_adr + 7]
            box_rot_dist_aft = ang_dist(array2quat(self.goal_rot), array2quat(box_rot_aft))
            ...
            reward = (box_rot_dist_bef - box_rot_dist_aft) / self.dt
            ...
            if not self.agent.training:
                print(f"Step Reward: {reward:.4f}, Ang Dist: {box_rot_dist_aft:.2f}, Ctrl Cost: {ctrl_cost_coeff * np.square(ctrl).mean():.4f}")
            if box_rot_dist_bef < 0.175:
                # reward += 10
                self.rot_count += 1
                print(f"Completed {self.rot_count} flips!")
                self.goal_rot = quat2array(add_rotation(self.goal_rot, self.np_random))
            ob = self._get_obs()
    ...
    def get_sim_obs(self):
        ...
        for i in range(rob_obs.shape[0]):
            if i == 0:
                self._cache_box_addrs()
                obs_i = [self.rob_box_dist, self.data.qpos[self.box_qpos_adr+3:self.box_qpos_adr+7], qvel[self.box_qvel_adr:self.box_qvel_adr+6]]
                # obs_i = [np.array([0,0,0]), self.data.qpos[-4:], qvel[-6:]]
            elif i == 1:
                goal_rot_dist = quat_dist(array2quat(self.goal_rot), array2quat(self.data.qpos[self.box_qpos_adr+3:self.box_qpos_adr+7]) )
                obs_i = [quat2array(goal_rot_dist), np.zeros(9)]
            else:
                obs_i = [np.zeros(13)]
            obs_i = np.concatenate(obs_i)
